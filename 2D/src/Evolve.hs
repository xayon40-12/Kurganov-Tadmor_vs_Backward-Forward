...
data System = Sys Func Func Func Func (Exp T) (Exp T) Evolve (Acc Arr)
...
arr :: Acc Arr
arr = generate (constant (Z :. width :. hight))
...
makePicture s =
  ...
  where
    ...
    hp = h2/2+3
    pics = [translate x y $
                bitmapOfArray (runN $ A.map toWord32 a) False
...
stepWorld _ s = [next sys | sys <- s]

next :: System -> System
next sys@(Sys f f' g g' dx dt c u) = Sys f f' g g' dx dt c $ u2
    where u2 = A.map (\v -> let (a,b) = unlift v in a+dt*b) $ A.zip u (c sys)
